...
function boxedContent(opts: {
  ...
  const indentStr = ' '.repeat(opts.indent ?? 0);
...
export class MessageRenderer {
  ...
  private renderAssistantMessage(msg: ChatMessage): string[] {
    const bw = this.getBoxWidth();
    const contentLines = msg.content.split('\n');
  ...
  private renderToolMessage(msg: ChatMessage): string[] {
  ...
  private renderErrorMessage(msg: ChatMessage): string[] {
    const bw = this.getBoxWidth();
    const contentLines = msg.content.split('\n');
    const b = BORDERS.box;
