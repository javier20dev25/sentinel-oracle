...
export function sevColor(sev: string): string {
  ...
  if (s === 'CRITICAL') return pc.red(sev);
  if (s === 'HIGH') return pc.yellow(sev);
  if (s === 'MEDIUM') return pc.cyan(sev);
  if (s === 'LOW') return pc.gray(sev);
  return sev;
}

export function sevColorFn(sev: string): (s: string) => string {
  const s = sev.toUpperCase();
  if (s === 'CRITICAL') return pc.red;
  if (s === 'HIGH') return pc.yellow;
  if (s === 'MEDIUM') return pc.cyan;
  if (s === 'LOW') return pc.gray;
  return (x: string) => x;
}
...
export function capabilityTag(type: string): string {
 ...
 */
export function attackChain(nodes: ChainNode[]): string {
 ...
 */
export function capabilityBars(caps: CapabilityCount[], maxWidth = 30): string[] {
  if (caps.length === 0) return ['(no capabilities detected)'];
...
export function severityPie(severities: Record<string, number>): string[] {
  ...
  for (const s of order) {
    ...
    const barFn: (s: string) => string = s === 'CRITICAL' ? pc.red : s === 'HIGH' ? pc.yellow : s === 'MEDIUM' ? pc.cyan : pc.gray;
...
export function fileHeatmap(files: FileFinding[], maxRows = 10): string[] {
  if (files.length === 0) return ['(no files with findings)'];
  ...
  return sorted.map(f => {
    ...
    const sevFn: (s: string) => string = f.severity === 'CRITICAL' ? pc.red : f.severity === 'HIGH' ? pc.yellow : pc.gray;
...
export function summaryBox(summary: ReportSummary): string {
  const width = 56;
  ...
  if (summary.scanTimeMs) lines.push(`  ${pc.bold('Scan time:')}    ${summary.scanTimeMs}ms`);
  if (summary.memoryMB) lines.push(`  ${pc.bold('Memory:')}       ${summary.memoryMB.toFixed(1)} MB`);
  if (summary.filesScanned) lines.push(`  ${pc.bold('Files:')}       ${summary.filesScanned}`);
...
export function welcomeBanner(provider?: string, model?: string): string {
  const banner = `
${pc.cyan('\u2554' + '\u2550'.repeat(52) + '\u2557')}
...
export function toolCard(name: string, params: string, status: 'running' | 'done' | 'error' | 'denied'): string {
...
export function insight(type: 'tip' | 'warning' | 'danger' | 'info', title: string, body: string): string {
  const labels: Record<string, string> = { tip: '[i]', warning: '[!]', danger: '[!!!]', info: '(i)' };
  const colors: Record<string, (s: string) => string> = { tip: pc.cyan, warning: pc.yellow, danger: pc.red, info: pc.gray };
  return [
...
export function permissionBannerText(toolName: string, params: string, mode: 'execute' | 'plan'): string {
  const w = 56;
...
export function findingsBox(title: string, lines: string[], severity?: string): string {
  const w = 58;
...
export function modeBanner(mode: 'execute' | 'plan' | 'auto'): string {
