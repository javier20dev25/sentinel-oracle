...
function handleMcpMessage(msg: JsonRpcMessage): JsonRpcResponse | null {
      ...
      try {
        ...
        switch (name) {
          case 'threat-query': {
            ...
            text = JSON.stringify(
              threats.length > 0 ? threats : { message: 'No threats found for this author', author: toolArgs.author },
              null, 2
            );
            break;
          }
...
export function startMcpServer(port = 3003): void {
...
export function startMcpHttpServer(port = 3003): void {
