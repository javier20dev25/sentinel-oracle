...
function ensureFile(): void {
  ...
  if (!fs.existsSync(RULES_FILE)) fs.writeFileSync(RULES_FILE, JSON.stringify([], null, 2), 'utf-8');
}
...
function writeRules(rules: Rule[]): void {
  ensureFile();
  fs.writeFileSync(RULES_FILE, JSON.stringify(rules, null, 2), 'utf-8');
}

export function addRule(name: string, instruction: string): void {
