...
export function wrapToolOutput(output: string, toolName: string): string {
  ...
  if (toolHadFindings) {
    for (const pattern of dismissalPatterns) {
      if (lowerResponse.includes(pattern)) {
        warnings.push(
          `⚠️ La respuesta de la IA dice "${pattern}" pero los tools encontraron evidencia de amenazas. ` +
...
// ─── Detección de intentos de inyección en código ────────────
...
const INJECTION_PATTERNS: { regex: RegExp; type: InjectionAttempt['type'] }[] = [
...
export function detectPromptInjection(code: string): InjectionAttempt[] {
...
export function formatInjections(attempts: InjectionAttempt[]): string {
