...
import { correlateFindings } from './threat_db.js';
...
export function getDefaultProvider() {
    ...
    if (!key && provider !== 'ollama')
        return null;
    try {
        return createProvider(provider, key, model);
    }
    catch (_a) {
        return null;
    }
...
function buildPlanModeResult(tcName, tcArgs) {
    return wrapToolOutput(`[PLAN MODE] Tool "${tcName}" would execute with arguments: ${JSON.stringify(tcArgs)}\nThe tool was NOT executed because Oracle is in plan mode. Explain what you would do and ask if they want to proceed.`, tcName);
}
export const streamingResult = { history: [] };
export function oracleChat(userInput_1, history_1, provider_1, onBeforeToolCall_1) {
...
export function oracleChatStream(userInput_1, history_1, provider_1, onBeforeToolCall_1) {
    return __asyncGenerator(this, arguments, function* oracleChatStream_1(userInput, history, provider, onBeforeToolCall, mode = 'execute') {
        ...
        while (iterations < MAX_TOOL_ITERATIONS) {
            ...
            for (const tc of pendingToolCalls) {
                ...
                const rawResult = runTool(tc.name, tc.arguments);
                executedTools.push({ toolName: tc.name, output: rawResult });
                yield yield __await(`${toolCard(tc.name, JSON.stringify(tc.arguments), 'done')}\n`);
