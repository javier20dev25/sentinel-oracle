...
class SystemAuditor {
    ...
    runDoctor() {
        return __awaiter(this, arguments, void 0, function* (deep = false) {
            console.log(pc.magenta('\n🩺 SENTINEL SYSTEM HEALTH REPORT'));
            console.log(pc.dim('   Scanning local workspace...\n'));
            const results = { packages: 0, critical: 0, suspicious: 0, findings: [] };
            ...
            for (const f of pkgFindings) {
                results.findings.push(Object.assign({ name: 'package.json' }, f));
            ...
            if (deep) {
                ...
                for (const depName of depNames) {
                    ...
                    catch (_unused) { /* skip permission errors */ }
                    for (const file of files.slice(0, 20)) { // Max 20 files/pkg for perf
                        try {
                            ...
                            for (const f of fnds) {
                                results.findings.push(Object.assign({ name: depName }, f));
                                if (f.severity === 'CRITICAL')
                                    results.critical++;
                                else if (f.severity === 'HIGH' || f.severity === 'MEDIUM')
                                    results.suspicious++;
    ...
    reportResults(results) {
        ...
        if (results.findings.length === 0) {
            ...
            for (const [name, fnds] of grouped) {
                const worst = fnds.reduce((w, f) => f.severity === 'CRITICAL' ? f : (w.severity === 'CRITICAL' ? w : f));
                ...
                if (fnds.length > 3)
                    console.log(pc.dim(`    ... and ${fnds.length - 3} more`));
            }
        }
        console.log(pc.dim('\n━━━━━━━━━━━━━━━━━━━━━'));
...
exports.SystemAuditor = SystemAuditor;
